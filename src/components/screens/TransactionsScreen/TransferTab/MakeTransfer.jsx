...
const MakeTransfer = ({ navigation }) => {
    const [transferForm, setFormInput] = useState({
        "Receipient Name": "",
        "Receipient Bank": "", 
    ...
    const formFields = Object.keys(transferForm);
    const firstname = useSelector(state => state.signup.users['Firstname'])
    const icons = ["user-circle", "bank", "money", "mobile-phone", "money"]
    
    const transferFormHandler = (field, value) => {
        let newFormDetails = transferForm;
    ...
    const dispatchTransfer = () => {
        dispatch(transferFunds(transferForm));
        setDispatchState(true);
    };

    const formError = useSelector(state => state.transfer.error );
    useEffect( () => {
    ...
    return(
        <View style={styles.container}>
            ...
            <View style={styles.footer}>
                <Text style={styles.form_title}>Funds transfer are easier than ever...</Text>
                ...
                <View style={styles.form}>
                    {
                        formFields.map((field, index) => (
                            <View style={styles.inputContainer} key={field}>
                                ...
                                <TextInput onChangeText={event => transferFormHandler(field, event)}
                ...
                <View style={styles.footerComponents}>
                    <Text style={styles.footer_text} color="tomato" title="Log out" >
                    ...
                    <View style={styles.btnGroup}>
                        ...
                        <View style={styles.button}>
                            <Button disabled={"" in transferForm ? true : false} color="cornflowerblue" title="Transfer" onPress={() => dispatchTransfer()}/>
                        </View>
                    </View>
                </View>
                <Text style={styles.footer_text2} color="tomato" onPress={() => dispatch(LogoutUser(firstname))}>
                    Log out
                </Text>
            </View>
        </View>
